...
export default function UpdateCodeModal({ open, onClose, place, onSaved }: Props) {
  ...
  // If closed or missing place, render nothing (prevents null access)
  if (!open || !place) return null;

  // Narrowed, non-null alias for TypeScript
  const p: Place = place;

  async function handleSave() {
    // Build payload depending on openAccess
    const payload:
      | {
          establishment_id?: string;
          place_id?: string;
    ...
    try {
      ...
      onSaved?.(openAccess ? "open_access" : code.trim());
      onClose();
    } catch (e: unknown) {
