...
export function CodePanel({ lat, lng }: { lat: number; lng: number }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const r = await fetch(`/api/resolve-building?lat=${lat}&lng=${lng}&radius=100`);
        const j = await r.json();
        if (j.status === "ok" || j.status === "code" || j.status === "no_code" || j.status === "no_restroom") {
          const c = j.candidates?.[0] || j.establishment && {
            place_id: j.establishment.google_place_id,
            name: j.establishment.name,
            address: j.establishment.address,
            lat: j.establishment.lat,
            lng: j.establishment.lng,
            code: j.code,
            code_updated_at: j.code_updated_at
          };
          if (c) {
            setCand(c);
            setCode(c.code ?? "");
          ...
          toast.error(j.error ?? "Resolve failed");
        }
      } catch (e: any) {
    ...
    try {
      const res = await fetch("/api/submit", {
        ...
        body: JSON.stringify({
          ...
          lng: cand.lng,
          code: code,
          staff_verified: false, // flip true for trusted users
        }),
      });
  ...
  return (
    <div className="w-full max-w-xl rounded-2xl border border-neutral-200 p-4 bg-white/80 backdrop-blur">
      <h3 className="text-lg font-semibold mb-2">Nearest place</h3>
      {loading && <div className="text-sm text-neutral-500">Finding building…</div>}
      {!loading && !cand && (
        <div className="text-sm text-neutral-600">No place found here.</div>
      )}
      {!loading && cand && (
        <>
          <div className="mb-3">
            <div className="text-base font-medium">{cand.name}</div>
            <div className="text-sm text-neutral-600">{cand.address ?? "No address"}</div>
            <div className="text-xs text-neutral-500">
              {cand.code ? <>Current code: <span className="font-mono">{cand.code}</span></> : "No code yet"}
              {cand.code_updated_at ? ` • Updated ${new Date(cand.code_updated_at).toLocaleString()}` : ""}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="flex-1 rounded-lg border border-neutral-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500"
            />
            <button
              onClick={submitCode}
              className="rounded-lg bg-emerald-600 text-white px-4 py-2 font-medium hover:bg-emerald-700"
            >
              {cand.code ? "Update code" : "Add code"}
            </button>
          </div>
        </>
      )}
